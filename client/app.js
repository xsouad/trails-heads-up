...
// The hat/Accessories cycle always treats its HIGHEST number as "no
// accessory" rather than needing a real blank image file -- so with hat: 4,
// values 1-3 are your actual accessory sprites and 4 just skips rendering
// that layer. If you add a real 4th accessory later, add it as hat_4.png and
// bump this count to 5 so 5 becomes the new "none" slot.
...
function renderAvatarStage(container, avatarObj) {
  ...
  ['base', 'face', 'hat'].forEach(layer => {
    // "No accessory" -- the top value in the hat cycle -- simply renders nothing.
    if (layer === 'hat' && avatarObj.hat === LAYER_COUNTS.hat) return;
    const img = document.createElement('img');
